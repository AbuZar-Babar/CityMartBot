...
// Clicking the current-company label opens "My Company" (#portalEntity)
// directly — there is no separate profile-panel "Change Company" link to
// wait for. So "dropdown open" now just means "the modal is visible".
async function openCompanyProfileDropdown(page, companyLabelText, timeoutMs = 12000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    if (await waitForModalVisible(page, PORTAL_ENTITY_MODAL_SELECTOR, 100)) return true;
    ...
    if (await waitForModalVisible(page, PORTAL_ENTITY_MODAL_SELECTOR, 2000)) return true;
  }
  return waitForModalVisible(page, PORTAL_ENTITY_MODAL_SELECTOR, 100);
}
...
// Generic: click whatever company name is currently showing in the top
// profile widget, instead of a hardcoded one — this now comes from
// switchCompanyByClick's own parameter instead of a fixed constant.

// ---------------------------------------------------------------------
// Company switching
// ---------------------------------------------------------------------

async function switchCompanyByClick(page, targetCompanyName, currentCompanyLabel) {
  ...
  // openCompanyProfileDropdown now waits until the "My Company" modal
  // itself is visible — clicking the label opens it directly, there's no
  // separate profile-panel "Change Company" anchor to click first.
  const modalOpened = await openCompanyProfileDropdown(page, currentCompanyLabel);
  if (!modalOpened) {
    await debugSnapshot(page, '03-my-company-modal-not-opened');
    return { ok: false, reason: `"My Company" modal did not open for "${currentCompanyLabel}"` };
  }
  await randomDelay(400, 700);
  await randomDelay(300, 600);
  ...
  try {
    ...
    let attempt = 0;
        const MAX_CLEAR_TYPE_ATTEMPTS = 3;
    ...
    while (attempt < MAX_CLEAR_TYPE_ATTEMPTS && !targetVisible) {
      ...
      // If typing never landed (field still empty), the unfiltered dropdown
      // still shows every company, so a page-wide text search can find a
      // false match even though nothing was actually typed. Only trust
      // "target visible" once the field itself confirms what we typed.
      if (!typedBack.includes(shortCode)) {
        console.log('  [switch] typing did not register in the field — retrying this attempt');
        continue;
      }
...
// companies.json holds richer objects ({ entityId, name, entityNo }) — convert
// each one into the "Name - entityNo" string the rest of the script expects.
const COMPANY_NAMES = JSON
  .parse(fs.readFileSync(path.resolve('companies.json'), 'utf8'))
  .map(c => `${c.name} - ${c.entityNo}`);
...
(async () => {
  ...
  console.log(`  [debug] currentLabel raw JSON: ${JSON.stringify(currentLabel)}`);
  ...
  for (const companyName of COMPANY_NAMES) {
    ...
    if (targetShortLabel === currentLabel) {
      ...
      switchResult = { ok: true };
            ...
            switchResult = await switchCompanyByClick(page, companyName, currentLabel);
